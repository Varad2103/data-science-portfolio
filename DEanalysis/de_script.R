...
## Load in data
data <- read.csv("D:/Downloads/merged_counts_table.csv", header=T, row.names=1) 

meta <- read.csv("D:/Downloads/sample_metadata.csv", header=T, row.names=1)
...
ggplot(data) +
  geom_histogram(aes(x = D1), stat = "bin", bins = 200) +
...
dds <- DESeqDataSetFromMatrix(countData = data, colData = meta, design = ~ batch + genotype + treatment)
...
plotPCA(rld, intgroup = c("genotype", "treatment", "batch"))
...
df <- cbind(meta, pca$x)
ggplot(df) + geom_point(aes(x = PC3, y = PC4, color = interaction(genotype, treatment)))
...
contrast_oe <- c("genotype", "WT", "KO")

res_tableOE_unshrunken <- results(dds, contrast=contrast_oe, alpha = 0.05)

resultsNames(dds)

res_tableOE <- lfcShrink(dds, coef="genotype_WT_vs_KO", type="apeglm")
